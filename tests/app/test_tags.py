from sample_manager.db.tag_repository import add_tag_to_sample, remove_tag_from_sample, get_all_tags
...
def test_remove_tag_from_sample(test_db):
    cursor = test_db.cursor()
    cursor.execute("INSERT INTO samples (filename, path) VALUES (?, ?)", ("snare.wav", "/samples/snare.wav"))
    sample_id = cursor.lastrowid
    test_db.commit()
...
def test_bulk_tag(test_db):
    cursor = test_db.cursor()
    cursor.execute("INSERT INTO samples (filename, path, extension) VALUES (?, ?, ?)", ("kick1.wav", "/s/k1.wav", "wav"))
    cursor.execute("INSERT INTO samples (filename, path, extension) VALUES (?, ?, ?)", ("kick2.wav", "/s/k2.wav", "wav"))
    test_db.commit()
