...
from sample_manager.db.sample_repository import create_sample, search_samples
...
def test_search_samples_tag_filter(test_db):
    ...
    cursor.execute(
        "INSERT INTO sample_tags (sample_id, tag_id) VALUES (?, ?)", (sample_id, tag_id)
    )
    conn.commit()
...
def test_search_samples_rating_filter(test_db):
    ...
    cursor.execute(
        "INSERT INTO ratings (sample_id, rating) VALUES (?, ?)", (id_good, 5)
    )
