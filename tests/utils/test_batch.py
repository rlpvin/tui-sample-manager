...
def test_rename_samples(test_db, tmp_path):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO samples (filename, path, extension) VALUES (?, ?, ?)", ("kick_01.wav", str(f1), ".wav"))
    cursor.execute("INSERT INTO samples (filename, path, extension) VALUES (?, ?, ?)", ("snare_01.wav", str(f2), ".wav"))
    conn.commit()
