...
def test_batch_rename_missing_file(test_db, tmp_path):
    ...
    cursor.execute("INSERT INTO samples (filename, path, extension) VALUES (?, ?, ?)", ("missing.wav", missing_path, ".wav"))
    conn.commit()
...
def test_batch_rename_destination_exists(test_db, tmp_path):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO samples (filename, path, extension) VALUES (?, ?, ?)", ("src.wav", str(f1), ".wav"))
    conn.commit()
...
def test_batch_normalize_temp_file_cleanup(test_db, tmp_path):
    ...
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("INSERT INTO samples (filename, path, extension) VALUES (?, ?, ?)", ("norm.wav", str(f1), ".wav"))
    conn.commit()
    ...
    with patch("subprocess.run", side_effect=Exception("FFmpeg failed")):
        ...
        # Check if temp file exists (it should be cleaned up by the finally/except block)
