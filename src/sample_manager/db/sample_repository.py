...
def create_sample(
    path, filename, extension, size, hash_val=None, bpm=0, musical_key=None, duration=0
):
    conn = get_connection()
    cursor = conn.cursor()
    ...
    try:
        cursor.execute(
            ...
            INSERT OR IGNORE INTO samples (
                path, filename, extension, size, hash, bpm, musical_key, duration
            )
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            """,
    ...
    if filters:
        ...
        if "tag" in filters and filters["tag"]:
            ...
            where_clauses.append(
                "s.id IN (SELECT sample_id FROM sample_tags st2 "
                "JOIN tags t2 ON st2.tag_id = t2.id WHERE t2.name LIKE ?)"
            )
