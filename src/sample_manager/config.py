...
def get_default_config():
    ...
    return {"database": {"path": str(Path(data_dir) / "samples.db")}, "directories": []}
...
def ensure_config():
    ...
    if not CONFIG_PATH.exists():
        ...
        # Since we only have tomllib (read-only in 3.11+), we'll do simple
        # string writing or just return the dict.
        # But let's actually write a basic version.
        try:
